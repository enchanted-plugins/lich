...
description: >
  ...
  Use when: mantis-core fires on a TypeScript/JavaScript file. Do not use
  for: non-TS/JS files; replacing biome; auto-fix mode (Mantis is advisory).
model: haiku
tools: [Read, Bash]
---
...
- One of these is available in the repo: `biome` binary, `tsc --noEmit` via local `typescript` devDep, or the Mantis fallback (minimal regex-based idiom checks — last resort).

## Inputs

- **Chained from mantis-core**: `{file: "Foo.tsx", substrate_status: "ok"|"parse-failed"}`
...
4. **Map rule IDs to M-engine categories.** From `config/biome-rule-map.json`:
   ...
   - `skip` — Reaper-overlap rules (noSecrets, noEval, noUnsafeOptionalChaining with security framing)
5. **Emit findings.** Append to `plugins/mantis-core/state/review-flags.jsonl`.

## Outputs

- Appends to `plugins/mantis-core/state/review-flags.jsonl`.
- Return: `{substrate: "biome"|"tsc"|"fallback-regex", rules_fired: N, skipped_reaper_overlap: N}`.

## Handoff

Findings flow through the standard mantis pipeline: mantis-core → mantis-sandbox (M1-class) → mantis-rubric → mantis-verdict.
...
- **F07 over-helpful substitution** — `biome check --apply`. Never. Mantis is advisory.
- **F13 distractor pollution** — on a 10k-line file, biome may emit hundreds of findings; filter to changed hunks (from Hornet's change classification) before mapping.
