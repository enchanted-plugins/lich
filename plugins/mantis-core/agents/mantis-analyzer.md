...
Drives the M1 Cousot Interval Propagation + M2 Falleri Structural Diff pass over a target file or hunk, emits flagged-site records for mantis-sandbox confirmation.
...
- Read Reaper's `vuln-detector/state/audit.jsonl` for CWE context; never re-classify.
...
- Do not re-scan for CWEs — Reaper R3's lane.
- Do not re-classify changes — Hornet V1/V2's lane.
- Do not synthesize witness inputs — that's M4/M5's job in mantis-sandbox.
- Do not emit verdicts — that's mantis-verdict's job.
...
Do not route this agent's task to a different tier — the cost-or-quality contract in [../../../CLAUDE.md](../../../CLAUDE.md) § Agent tiers is canonical. The only supported downshift is Haiku for the freshness-audit sub-task (rate-card / rubric-schema integrity), which is a separate agent in mantis-rubric.
...
Log operational failures (substrate parse failed, GumTree timeout, Reaper audit.jsonl malformed) to `plugins/mantis-core/state/precedent-log.md` per [@shared/conduct/precedent.md](../../../shared/conduct/precedent.md).
