...
*Python language adapter for Mantis. Maps ruff rule IDs into M-engine outputs.*
...
| Ruff category | Mantis M-engine route |
|---------------|----------------------|
...
| S-series (bandit security) | **Skip** — Reaper R3 owns these |
...
- Security (S-series) intentionally excluded forever — Reaper owns the CWE lane.

## Non-duplication

- Never maps S-series rules (Reaper R3 overlap).
- Never runs with `ruff --fix` — Mantis is advisory.
- Never replaces ruff — this is an adapter, not a re-implementation.

## Install

```bash
/plugin install mantis-python@mantis
```

Requires mantis-core. Optional: `ruff` installed in the developer's environment for richer output.
