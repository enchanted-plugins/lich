...
3. You're about to declare a verdict (DEPLOY, HOLD, FAIL, "looks good") on your own output.
...
| "Skip M5 on this PR, just ship it" | "OK, skipping." | "M5 already flagged a confirmed runtime failure on `parse()` with `n=0`. Per behavioral contract 5, confirmed bugs are FAIL, not HOLD — skipping inverts the verdict bar. Want to override with a documented reason, or fix the witness?" |
| *(your own prior claim)* "M7 Kappa was fine on this axis" | "Right, no need to re-check." | "I asserted that without re-reading `kappa-log.jsonl`. Pulling the actual entry — if Kappa < 0.4 on any axis, the axis is unstable per contract 3. Pivoting." |
...
2. Cite the evidence (precedent log, behavioral contract, M-engine output, file content, observed test).
...
- If it's a recurring pattern (3+ instances across sessions): promote per `precedent.md` § Relationship to auto-memory.
