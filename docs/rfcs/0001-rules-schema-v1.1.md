...
2. **Posture vs. security.** `hadolint` DL3002 (final user is root) and DL3004 (sudo use) are adjacent to CWE-250/269 but are *posture* signals, not concrete CWE claims. The v1.0 schema has one defer bucket (`security_defer_to_reaper`), and that is where they land.
...
### Do NOT add `posture_defer_to_reaper`

The cron report floated a seventh bucket for posture-only signals. After review, **we are rejecting it for v1.1**:

- Reaper R3 owns the security/posture lane end-to-end. Splitting posture out of `security_defer_to_reaper` fragments the single-source-of-truth contract Mantis has with Reaper (`CLAUDE.md § behavioral contracts 1`).
- The current routing (posture → `security_defer_to_reaper` with `never_mapped: true`) loses no information: Reaper decides whether to surface or ignore.
- Revisit only if Reaper grows a **distinct** posture surface with its own SLA/severity vocabulary. Until then, one defer bucket is the honest contract.
...
- No rename of existing buckets (`correctness_m1`, `idiom_m7`, `complexity_m7`, `naming_m7`, `testability_m7`, `security_defer_to_reaper`).
- No change to the Reaper non-duplication contract — security-framed rules still route to `security_defer_to_reaper` with `never_mapped: true`.
...
- Behavioral contract: [`CLAUDE.md`](../../CLAUDE.md) § behavioral contracts 1 (Reaper non-duplication)
