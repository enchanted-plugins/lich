# Mantis — Product Architecture

*Phase 3 · Plugin #6 · enchanted-plugins · answers the developer's sixth question: "Is this code good?"*

**Name.** Mantis. After the Mantis Lords of Hollow Knight — gate-reviewers who judge worthiness through trial before letting you pass. Every PR is a supplicant at the gate; every engine is a test the code must survive. Mantis joins Hornet (change comprehension) and Weaver (git flow) in the Hollow Knight cluster — three HK entities for three related dev-surface plugins is intentional brand signal. This slot previously carried the placeholder name "Athena"; that name is retired because Athena is a pre-existing mythological figure Supergiant borrowed, not a game-native entity. Mantis Lords are game-native and pass the naming convention.

**Engine prefix.** M (single letter, unique across F/A/V/S/W/L).

**Trigger model.** Hybrid. PostToolUse hook subscribes to Hornet's change-classification signal (Phase 1: file-tail of `hornet/plugins/change-tracker/state/audit.jsonl`; Phase 2: `hornet.change.classified` event via enchanted-mcp) and auto-reviews affected hunks. Skill-invoked commands (`/mantis-review`, `/mantis-explain`) are the manual handle for ad-hoc deep reviews. Silent on DEPLOY, surfacing on HOLD/FAIL.
...
**Recommendation.** Stdlib `resource.setrlimit` + `signal.alarm` on Unix, exposed as the `mantis-sandbox` sub-plugin. Exact caps at launch:
...
**Recommendation.** MVP (Phase 1) uses boundary-value synthesis driven directly by M1 flags: for each flagged variable, Mantis tries a per-type default set (`{0, -1, None, "", [], sys.maxsize}` for Python int/str/list), runs the containing function in M5's sandbox, catches `ZeroDivisionError`, `TypeError`, `IndexError`, `OverflowError`. If Hypothesis is installed in the developer's environment, Mantis's adapter detects it and upgrades to ghostwriter-synthesized stubs (Phase 2 engine M4 — gated behind Hypothesis availability). CrossHair is the Phase 3 moonshot, behind the developer's explicit opt-in.
...
**Recommendation.** Beta-Binomial Thompson sampling. Initial prior `Beta(α=1, β=1)` uniform for all (developer, rule) pairs. Update rule: accept → α++; reject → β++; overrode → treat as 0.5 accept + 0.5 reject (ambiguous signal, update both). Surface probability on each review pass is a Thompson sample from the posterior. Minimum sampling floor: 5% (if the Thompson sample would surface below 5%, upgrade it to 5%). This prevents a rule from being "dead" after a few rejections; the developer can permanently suppress via `/mantis-disable <rule>` which writes to an overrides file. Overrides auto-expire with a quarterly "still disabled?" prompt — anti-drift protocol.

Persistence: `plugins/mantis-preference/state/learnings.json` — one file per developer ID, containing a map `{rule_id → {alpha, beta, surface_count, accept_count, reject_count, last_update}}`. Atomic writes via `os.rename` (brand standard A4 Atomic State Serialization pattern).
...
Model tier: Sonnet default judge. Haiku when Nook's `nook.budget.threshold.crossed` fires at 80% (cost-aware downshift, brand-standard cost contract). Opus reserved for disagreement adjudication only — cost-contract respected.

**Pitfall.** Position bias is real even with a single swap — a 2-sample average is still noisy. Mantis's Kappa reporting is the honest-numbers floor: if Kappa is unstable, the axis output is explicitly flagged rather than averaged silently. Preference expert pushed for 8 axes; static-analysis researcher pushed for 3. Compromise: 5 axes at launch, Phase 2 survey data from real review flows informs whether to expand or consolidate. Source: [MT-Bench (Zheng et al. 2023)](https://arxiv.org/abs/2306.05685).

---

## Layer 8: Verdict Contract — DEPLOY / HOLD / FAIL Thresholds

**Prior art.** Flux's CLAUDE.md defines the three-verdict vocabulary: DEPLOY (σ < 0.45 AND overall ≥ 9.0 AND all 5 axes ≥ 7.0 AND 8/8 SAT assertions pass), HOLD (any axis < 7.0 or σ ≥ 0.45), FAIL (reviewer flags a structural issue). Mantis adopts the same three-verdict shape but defines its own threshold math because the axes and assertions differ.
...
| **HOLD** | 1-2 HIGH flags AND no CRITICAL | Any timeout-without-confirmation | ≥ 50% posterior > 0.3 | Any axis < 3.5 OR Kappa < 0.4 | Surface to reviewer with per-engine breakdown; Weaver's pre-commit gate warns |
| **FAIL** | Any CRITICAL flag OR ≥ 3 HIGH flags | Any confirmed runtime failure | (N/A — posterior doesn't downgrade from HOLD to FAIL) | Any axis ≤ 2/5 OR > 2 axes < 3 | Block; Weaver's pre-commit gate refuses auto-commit; developer acknowledgment required |

**Pitfall.** A single M5-confirmed runtime failure (e.g., div-by-zero with a concrete witness) is load-bearing evidence — it's not a probability, it's a fact. The verdict math treats confirmed failures as hard FAIL triggers, not soft scores. Inversely, M7 rubric scores are genuinely subjective; they inform HOLD but can never single-handedly fail a PR. This asymmetry is intentional and documented. Source: [Flux CLAUDE.md § DEPLOY bar](../../flux/CLAUDE.md).

---

## Layer 9: Reaper, Hornet, Allay, Nook, Weaver Integration Contract

**Prior art.** The enchanted-plugins ecosystem uses per-plugin `audit.jsonl` files as the Phase 1 source of truth (enchanted-mcp event bus is Phase 2). Reaper's `plugins/vuln-detector/state/audit.jsonl` records are shape `{event:"vuln_detected", ts, file, line, vuln_id, cwe, severity, description, language, tool}` (verified via direct file inspection). Hornet's `plugins/change-tracker/state/audit.jsonl` records classify changes. Allay publishes token/cost metrics to `plugins/*/state/metrics.jsonl`. Nook (building) will publish budget-threshold events. Weaver's pre-commit gate already subscribes to `nook.budget.threshold.crossed` and will subscribe to `mantis.review.completed` per brand-standard event-envelope convention.
...
**Recommendation.** Phase 1 uses file-tailing of sibling `audit.jsonl` files with a pinned record-shape contract in Mantis's `hooks/record-shapes.json`. Phase 2 migrates to MCP event subscription with the same payload shape.

**Record shapes Mantis consumes (Phase 1):**

```json
// reaper/plugins/vuln-detector/state/audit.jsonl → used as M6 attention weight
...
// hornet/plugins/change-tracker/state/audit.jsonl → used to detect what Mantis should review
{"event":"change.classified","ts":"2026-04-19T12:34:57Z","file":"src/api.ts",
 "classification":"behavior-change","trust_score":0.62,"hunks":[...]}
```

**Record shape Mantis emits (`plugins/verdict/state/verdict.jsonl`):**
...
- Mantis NEVER re-scans for CWE-89 SQL injection, CWE-79 XSS, CWE-918 SSRF, or any other CWE-tagged security finding. These are Reaper R3's lane. If Reaper's audit.jsonl already has a CRITICAL CWE on the file, Mantis *increases review-attention weight* for M6 and adds a "Security context: Reaper flagged {cwe} {severity}" note to M7's rubric input, but does not re-classify the finding.
- Mantis NEVER re-classifies a change. Hornet's V1 Semantic Diff + V2 Bayesian Trust output is authoritative. Mantis is a *consumer*, not a peer classifier.
- Mantis consumes but does not mutate Allay's token metrics. M7 judge-tier downshift to Haiku under Nook budget pressure is the only cross-plugin control flow Mantis exercises.
...
|-------|---------|---------|-------------------|
| `mantis.review.completed` | End of PostToolUse Mantis review | `{pr_id, verdict, engines{M1..M7}, rubric_scores, kappa}` | Weaver pre-commit gate, Nook cost attribution, audit dashboard |
| `mantis.rule.disabled` | Developer runs `/mantis-disable <rule>` | `{developer_id, rule_id, ts, expiry_ts}` | Preference-accumulator archival; quarterly re-prompt scheduler |
| `mantis.sandbox.failed` | M5 sandbox errored (not a bug — infra failure) | `{file, function, error_class, retry_budget}` | Observability; never inflates a verdict |

**Subscribes:**

| Event | Source | Effect |
|-------|--------|--------|
| `hornet.change.classified` | Hornet | Triggers Mantis review on hunks above a configurable trust threshold |
| `reaper.vuln.detected` | Reaper | Boosts M6 attention weight for the affected file; adds security context note to M7 input |
| `nook.budget.threshold.crossed` | Nook | Downshifts M7 judge from Sonnet → Haiku at 80% budget |
| `allay.runway.threshold.crossed` | Allay | Pauses M5 sandbox runs (CPU budget conservation) until runway recovers |

---

## Layer 10: Sub-Plugin Breakdown & Developer Query Path

**Prior art.** Allay-style marketplace: one sub-plugin per engine OR one per orthogonal concern (language adapter, cross-plugin router, developer-UX surface). Reaper ships 5 sub-plugins (secret-scanner, vuln-detector, action-guard, config-shield, audit-trail); Weaver ships 5 similarly organized. The meta `full` plugin pulls all siblings via dependency resolution.
...
| 7 (above + mantis-python + mantis-typescript) | Medium-high | Simple | Adapters are the right surface but add install ceremony |
...
|------------|------|----------|
| `mantis-core` | M1 Cousot Interval Propagation + M2 Falleri Structural Diff + verdict synthesizer | analyzer |
| `mantis-sandbox` | M5 Bounded Subprocess Dry-Run (Unix-only at launch) | analyzer |
| `mantis-preference` | M6 Bayesian Preference Accumulation | learning |
| `mantis-rubric` | M7 Zheng Pairwise Rubric Judgment | judgment |
| `mantis-python` | Python language adapter (ruff rule-ID mapping, Python-specific idioms) | adapter |
| `mantis-typescript` | TypeScript language adapter (biome rule-ID mapping, TS-specific idioms) | adapter |
| `mantis-verdict` | Cross-sub-plugin verdict composition + event emission | router |
| `full` | Meta-plugin pulling all 7 via dependency resolution | meta |

Developer surfaces Day 1:

- **Skill**: `/mantis-review <hunk|file|PR>` — on-demand deep review
- **Skill**: `/mantis-explain <finding_id>` — walks through why M1/M5/M7 flagged something
- **Skill**: `/mantis-disable <rule_id>` — permanent suppression with auto-reprompt quarterly
...
**Pitfall.** Every sub-plugin is a maintenance surface. 7 is the upper bound of what a two-week MVP can credibly scaffold *and* keep healthy. If any sub-plugin ships without tests or a clear owner-module, it drags the whole plugin's reliability. Plugin-brand architect overrode preference expert's push for 9 sub-plugins (adding separate rubric-judge and rubric-aggregator); collapsed into mantis-rubric to stay within scope.
...
| M7 judge tiers | Sonnet default, Haiku under Nook budget, Opus adjudication | Cost contract with Nook | Uncontrolled Opus spend |
| Verdict | DEPLOY/HOLD/FAIL with hard floors + composite | Interpretable + catches both kinds of bugs | Weighted-score vibes |
| Reaper integration | File-read of `vuln-detector/state/audit.jsonl` (P1), event (P2) | Non-duplication of R3 | Double-reporting CWEs |
| Hornet integration | Subscribe to `hornet.change.classified` | Consumer, not re-classifier | Peer-level drift |
| Sub-plugin count | 7 + `full` meta | Allay-style sliceable granularity | Over-slicing |
...
│   │   ├── agents/mantis-analyzer.md
│   │   ├── commands/mantis-review.md
│   │   ├── hooks/hooks.json
│   │   ├── skills/mantis-review/SKILL.md
│   │   ├── state/{learnings.json,precedent-log.md}
│   │   └── README.md
│   ├── mantis-sandbox/
│   │   ├── .claude-plugin/plugin.json
│   │   ├── agents/mantis-sandbox-runner.md
│   │   ├── hooks/hooks.json
│   │   ├── skills/mantis-sandbox/SKILL.md
│   │   ├── state/
│   │   └── README.md
│   ├── mantis-preference/
│   │   ├── .claude-plugin/plugin.json
│   │   ├── agents/mantis-preference-updater.md
│   │   ├── hooks/hooks.json
│   │   ├── skills/mantis-disable/SKILL.md
│   │   ├── state/{learnings.json,overrides.json}
│   │   └── README.md
│   ├── mantis-rubric/
│   │   ├── .claude-plugin/plugin.json
│   │   ├── agents/mantis-judge.md
│   │   ├── config/rubric-v1.json
│   │   ├── hooks/hooks.json
│   │   ├── skills/mantis-explain/SKILL.md
│   │   ├── state/kappa-log.jsonl
│   │   └── README.md
│   ├── mantis-python/
│   │   ├── .claude-plugin/plugin.json
│   │   ├── hooks/hooks.json
│   │   ├── skills/mantis-python/SKILL.md
│   │   ├── config/ruff-rule-map.json
│   │   └── README.md
│   ├── mantis-typescript/
│   │   ├── .claude-plugin/plugin.json
│   │   ├── hooks/hooks.json
│   │   ├── skills/mantis-typescript/SKILL.md
│   │   ├── config/biome-rule-map.json
│   │   └── README.md
│   ├── mantis-verdict/
│   │   ├── .claude-plugin/plugin.json
│   │   ├── agents/mantis-verdict-synthesizer.md
│   │   ├── hooks/hooks.json
│   │   ├── state/verdict.jsonl
│   │   └── README.md
│   └── full/
...
│   ├── architecture/
│   │   ├── generate.py
│   │   ├── mantis-architecture.md    ← this document
│   │   ├── highlevel.mmd
│   │   ├── dataflow.mmd
│   │   ├── lifecycle.mmd
│   │   ├── hooks.mmd
...
### Mantis Named Engines

| ID | Name | Sub-plugin | Algorithm | Source |
|----|------|-----------|-----------|--------|
| M1 | Cousot Interval Propagation | mantis-core | Abstract interpretation over interval + nullability + container-shape lattices with threshold widening | Cousot & Cousot POPL'77 |
| M2 | Falleri Structural Diff | mantis-core | GumTree two-phase AST matching (top-down hash + bottom-up Dice) | Falleri et al. ASE'14 |
...
| M5 | Bounded Subprocess Dry-Run | mantis-sandbox | Stdlib `resource.setrlimit` + `signal.alarm` + subprocess sandbox | Python stdlib; novel composition for code review |
| M6 | Bayesian Preference Accumulation | mantis-preference | Beta-Binomial Thompson sampling per (developer, rule) with 5% minimum sampling floor | Thompson 1933; Russo & Van Roy 2018 |
| M7 | Zheng Pairwise Rubric Judgment | mantis-rubric | 5-axis rubric + position-swap debiasing + Cohen's Kappa inter-judge reliability | Zheng et al. MT-Bench 2023 |
...
|-------|---------|---------------|-------------------|
| `mantis.review.completed` | End of Mantis review pass | `{pr_id, verdict, engines, rubric_scores, kappa}` | Weaver pre-commit gate; Nook attribution |
| `mantis.rule.disabled` | Developer `/mantis-disable` | `{developer_id, rule_id, expiry_ts}` | Preference archival; re-prompt scheduler |
| `mantis.sandbox.failed` | M5 infra failure (not a finding) | `{file, function, error_class}` | Observability |

**Subscribes:**

| Event | Source | Effect on Mantis |
|-------|--------|------------------|
| `hornet.change.classified` | Hornet | Trigger review on affected hunks |
| `reaper.vuln.detected` | Reaper | Boost M6 attention weight; annotate M7 input |
| `nook.budget.threshold.crossed` | Nook | Downshift M7 judge (Sonnet → Haiku) at 80% |
| `allay.runway.threshold.crossed` | Allay | Pause M5 sandbox runs until runway recovers |
...
| Time-of-check-to-time-of-use (TOCTOU) | — | — | ranks | partial | Out of MVP; Reaper's R3 catches security-flavored TOCTOU; Mantis covers dataflow variant in Phase 2 |

(Note: **CWE-tagged security sinks** — SQL injection, XSS, SSRF, etc. — are explicitly Reaper's R3 lane, not Mantis's. The row is omitted here because it's not a Mantis responsibility.)

### Language Adapter Contract

| Adapter | Top linter mapped | Rule count at launch | Universal rules (from common core) | Language-specific rules |
|---------|-------------------|---------------------|-----------------------------------|------------------------|
| `mantis-python` | ruff (astral-sh) | ~120 mapped / ~900 available | 40 (dead code, complexity, naming, duplicates) | 80 Python-only (pyupgrade idioms, list/dict-comp, async-await patterns, typing modernization) |
| `mantis-typescript` | biome (biomejs) | ~80 mapped / ~423 available | 35 (same universal core) | 45 TS-only (hooks exhaustive deps, JSX a11y, `any`-avoidance, narrow-type-guards) |
| *(Phase 2)* `mantis-rust` | clippy | ~80 mapped / ~550 available | 35 (same core) | 45 Rust-only (borrow idioms, lifetime placement, `clone` avoidance) |
| *(Phase 2)* `mantis-go` | golangci-lint | ~40 mapped / ~100 aggregator | 30 (same core) | 10 Go-only (err-check-pattern, context-propagation) |
| *(Phase 2)* `mantis-java` | Checkstyle + PMD | ~80 mapped / ~600 total | 40 | 40 Java-only (Effective Java items) |
...
```json
{
  ...
  "rules": {
    ...
    "mantis-core:M1-division-by-zero": {
      "alpha": 8,
      "beta": 2,
      "surface_count": 10,
      "accept_count": 7,
...
```json
{
  ...
  "prompt_template_ref": "plugins/mantis-rubric/skills/mantis-explain/rubric-prompt.md"
}
```
...
| HOLD | 1-2 HIGH flags, no CRITICAL | Any timeout-without-confirmation | ≥ 50% surfaced posterior > 0.3 | Any axis < 3.5 OR Kappa < 0.4 | Surface to reviewer; Weaver warns |
| FAIL | Any CRITICAL OR ≥ 3 HIGH flags | Any confirmed runtime failure | (N/A — posterior doesn't failure-downgrade) | Any axis ≤ 2 OR > 2 axes < 3 | Block; Weaver refuses auto-commit |
...
- Languages: `mantis-python` + `mantis-typescript` only.
- Platform: Unix-only for M5; Windows skips M5 with honest note.
- Integration: File-based reads from Reaper/Hornet audit.jsonl; no MCP yet.
- Surfaces: `/mantis-review`, `/mantis-explain`, `/mantis-disable`, PostToolUse hook, status-line badge, PDF report.
...
- Languages added: `mantis-rust` + `mantis-go` + `mantis-java` + `mantis-kotlin`.
- Platform: M5 Windows support via Job Objects backend.
- Integration: Migrate file-reads to MCP event bus (`hornet.change.classified`, `reaper.vuln.detected`, `nook.budget.threshold.crossed`).
...
- **Lifecycle** — hybrid trigger. PostToolUse hook (Write|Edit|MultiEdit) drives `mantis-core`, `mantis-sandbox`, `mantis-preference` passes. SessionStart hook (rare — only when `config/rubric-v1.json` needs refresh). Skill commands (`/mantis-review`, `/mantis-explain`, `/mantis-disable`).
- **Algorithms** — M1–M7 named engines; M1+M2 in mantis-core, M5 in mantis-sandbox, M6 in mantis-preference, M7 in mantis-rubric. Defining engine: **M5 Bounded Subprocess Dry-Run** (the novel pipeline moat).
- **Behavioral contracts**:
  1. **[H] IMPORTANT — Mantis never re-scans CWE-tagged security findings.** If Reaper's audit.jsonl has a finding on the file, Mantis boosts attention weight but does not re-classify. This is the non-duplication contract with Reaper R3.
...
- **State paths** — runtime (gitignored): `plugins/mantis-core/state/`, `plugins/mantis-sandbox/state/run-log.jsonl`, `plugins/mantis-preference/state/{learnings.json,overrides.json}`, `plugins/mantis-rubric/state/kappa-log.jsonl`, `plugins/mantis-verdict/state/verdict.jsonl`. Ship-time config (committed): `plugins/mantis-rubric/config/rubric-v1.json`, `plugins/mantis-python/config/ruff-rule-map.json`, `plugins/mantis-typescript/config/biome-rule-map.json`.
- **Agent tiers** — Opus = M7 disagreement adjudication + cross-engine verdict synthesis; Sonnet = M7 default judge + mantis-core analyzer loops; Haiku = M7 budget fallback + rubric-schema freshness audit + M2 structural summarization.
- **Anti-patterns** — duplicating Reaper R3; silent M5 skip on Windows; rule-death from single rejection (M6 floor violation); bare M7 score without Kappa; unbounded sandbox.

### Handoff to Schematic

| Architecture decision | Schematic placeholder | Fill value |
|----------------------|----------------------|-----------|
| Plugin slug | `{{PLUGIN_SLUG}}` | `mantis` |
| Display name | `{{PluginName}}` | `Mantis` |
...
| Game origin | `{{PLUGIN_GAME_ORIGIN}}` | `Hollow Knight — Mantis Lords (gate-reviewers)` |
...
| First sub-plugin | `{{SUB_PLUGIN_1_NAME}}` | `mantis-core` |
| Sub-plugin count | `{{SUB_PLUGIN_COUNT}}` | `7` + `full` meta |
| Trigger model | `{{TRIGGER_MODEL}}` | `hybrid` (PostToolUse hook + skill-invoked) |
| Events published | `{{EVENT_PUBLISH_LIST}}` | `mantis.review.completed, mantis.rule.disabled, mantis.sandbox.failed` |
| Events subscribed | `{{EVENT_SUBSCRIBE_LIST}}` | `hornet.change.classified, reaper.vuln.detected, nook.budget.threshold.crossed, allay.runway.threshold.crossed` |
| Repo URL | `{{REPO_URL}}` | `https://github.com/enchanted-plugins/mantis` |
| Plugin home dir | `{{PLUGIN_HOME_DIR}}` | `~/.claude/plugins/mantis` |

**Placeholder gaps (new tokens to propose adding to schematic):** none. The architecture's 7-sub-plugin breakdown and 5-axis rubric fit within schematic's existing token set — though the `{{SUB_PLUGIN_2_*}}` through `{{SUB_PLUGIN_7_*}}` tokens aren't defined in schematic yet (schematic currently only enumerates sub-plugin 1 placeholders, per nook-architecture's same observation). Recommendation: extend schematic's vocabulary to `{{SUB_PLUGIN_N_*}}` with a max N of 8.

---

*Generated 2026-04-19. Source prompt: `flux/prompts/mantis-architecture/prompt.xml` v1. Review workflow: `/test-prompt` → `/converge` → dispatch. Next step after this document: execute `/create` pass to fill remaining schematic placeholders into working plugin code.*
