...
- **`generate.py` raises KeyError**: a plugin's `plugin.json` is missing a required field. Compare against any sibling plugin (`enchanter-ai/wixie/plugins/prompt-crafter/.claude-plugin/plugin.json`).
